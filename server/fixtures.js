if (Posts.find().count() === 0) {
  ...
  Posts.insert({
    ...
    categorias: Categorias.find({nome:'humildade'}, {_id:1}).map(function(item){ return item._id; })
  });
  ...
  Posts.insert({
    ...
    categorias: Categorias.find({nome:'blog'}, {_id:1}).map(function(item){ return item._id; })
  });
}else{
